...
# JASA Paper
# We define our regresion operator as a function of Y_c

evaluate_regression_at_query_v3 <- function(M_hat, y_c_strata, query_y_c,
                                            eigenfunctions, gamma_c, p) {
  ...
  # GOAL: evaluate regression at query
  #
  #       do steps 6, 7, and 8 all together
  #       in the JASA method, we include y_c's effect on the cross-covariance term immediately
  #       so we don't apply the y_c weight here
  #
  #
  # Input: 
  #
  # - M_hat                  (p^2 length list where each entry is 'i_j') 
  # - y_c_strata             (n_stratum x q_c matrix)
  # - query_y_c              (q_c dim vector)
  # - eigenfunctions         (list of p matrices, each of which is m x d_i)
  # - gamma_c                (number)
  # - p                      (integer)
  ...
  for (i in 1:p) {
    for (j in i:p) {
      ...
      # recall M_hat[[key]] = n x d x d
      # flatten it to be n x (d^2)
      M_hat_ij_flat <- matrix(M_hat[[key]], nrow = n_stratum, ncol = max_components^2) 
      M_at_query <- apply(M_hat_ij_flat, 2, sum) # (comps^2) length vector
