# main algorithm functions to be put in the running script

library(dplyr)
main_algorithm <- function(ews, ew_symb, movements, task_num, tn_symb, tns, min_edgges, ncores){
  
  # ------------------------------------------------------------------------
  # 
  # single function to carry out a fitting task
  # 1) construct folder name and search through all files in the folder
  # 2) preprocess to get data_df4
  # 3) fit
  # 4) save
  #
  # input:
  # 
  # - ews          (vector of numbers):  epoch or week numbers in vector form
  # - ew_symb      (character):          'e' for epoch or 'w' for week
  # - movements    (vector of numbers):  which movements to keep track of
  # - task_num     (string):             '04'
  # - tn_symb      (character):          't' for time scale or 'n' for number of replicates
  # - tns          (number):             number to denote time scale or number of replicates
  # - min_edges    (number):             find a graph with at least this many edges
