...
source("/u/home/j/jasenzz/Graphical_Cox_Process/GMpp-main/jasen_functions/9_visualize.R")
...
for(i in 1:length(IDs)){
    ...
    ntrain <- max(data_df2$subject_num)
    p <- max(data_df2$feature_id)
    ...
    quantile(data_df2[,(.N),by=c("subject_num","feature_id")]$V1)
    
    Rmat_diag_full = get_rho_diag_pp(data_all=data_df2,
    ...
    res_GPP_BIC <- run_GPP_HT_BIC_tol(Rmat_IC, grpind, ntrain, factor)
    ...
    g_adj <- adjacency_heatmap(graph_all[["GPP_BIC"]])
