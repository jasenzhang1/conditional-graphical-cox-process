...
# BIC 
factor = sqrt(ntrain)
res_GPP_BIC = run_GPP_HT_BIC_v2(Rmat_IC, grpind, ntrain, factor, min_edges)
graph_all[["GPP_BIC"]] = as.matrix((get_groupNorm(res_GPP_BIC, grpind)!=0)+0)
res_all[["GPP_BIC"]] = res_GPP_BIC

graphs_1 <- gpp_heatmap(res_GPP_BIC, grpind, graph_true)
...
# BIC and AIC
factor = sqrt(ntrain)
res_count_BIC = run_GPP_HT_BIC_v2(S_count, grpind, ntrain, factor, min_edges)
graph_all[["count_BIC"]] = as.matrix((get_groupNorm(res_count_BIC, grpind)!=0)+0)
res_all[["count_BIC"]] = res_count_BIC

graphs_2 <- gpp_heatmap(res_count_BIC, grpind, graph_true)
