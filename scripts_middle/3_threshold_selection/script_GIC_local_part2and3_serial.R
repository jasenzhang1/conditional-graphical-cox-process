...
if(model_type == 'mice'){
  ...
  min_connect_pcts <- as.numeric(args[11:length(args)])
  ...
  temp_file_dir <- paste0(temp_file_dir, '/GIC_local_', ID, '_', discrete_level, '_t', time_scale, '_nquery', cont_ind)
  mouse <- T
...
GIC_step2and3_serial_tau_c(temp_file_dir, suffix_name, k, min_connect_pcts)
