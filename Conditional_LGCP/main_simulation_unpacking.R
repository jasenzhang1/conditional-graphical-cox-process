...
grid.arrange(graph_results_i$estimated_graphs_part_1$g_01)

n <- 500
query_id <- 1
unpacking_pipeline('simu_results_a_sparse_v2_OG', '112', time_grid_est, time_grid, n, p, query_id)

# convergence of intermediate estimators

metrics <- c('rho_i_dist', 'rho_ij_dist', 'g_ij_dist', 'C_HS', 'P_HS', 'auc')
grid.newpage()
metrics_summary <- visualize_metrics('simu_results_a_sparse_v2_OG', metrics, 1, 2)
