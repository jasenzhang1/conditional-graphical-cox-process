...
age_query <- 1:19/20
timestamp_query <- 0:4/4
query_y_cs_both <- data.frame(rep(age_query, length(timestamp_query)),
                              rep(timestamp_query, each = length(age_query))) # age, timestamp

colnames(query_y_cs_both) <- c('age', 'timestamp')
query_y_cs_age <- data.frame(age = age_query)
  

query_y_cs <- list(query_y_cs_both, query_y_cs_age)
...
for(i in 1:length(IDs)){ 
  ...
  # 2) obtain discrete strata from y_d_df
