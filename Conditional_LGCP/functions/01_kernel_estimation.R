...
truncNorm_denom <- function(t=NULL, gamma=1 ,a=0, b=1){
  ...
  # output:
  #
  # - w_h(t)  (vector of times)   each element corresponds to the correction factor at t_i
  # 
...
estimate_density <- function(t_event, t_seq){
  ...
  # -----------------------------------------------
  # GOAL: obtain rho_i(t) intensity estimate
  #
  # Input:
  #
  # t_event (3805-dim vector)         all timestamps for mark i for subject k 
  ...
  kernel_evals <- gaussian_kernel(t_seq, t_event, gamma) # m x n matrix
...
estimate_bivariate_density <- function(event_times_i, event_times_j, 
  ...
  # estimate \Gamma_{i, j}^k(s,t) for given processes i and j
  #
  #
  #
  # input:
  ...
  for (i in 1:length(eval_grid_s)) {
    for (j in 1:length(eval_grid_t)) {  # for each point in the grid
      
      s <- eval_grid_s[i]
      t <- eval_grid_t[j]
      
      ks_vec <- gaussian_kernel(s, event_times_i, gamma_i)    # n_i dim vec
      kt_vec <- gaussian_kernel(t, event_times_j, gamma_j)    # n_j dim vec
      ...
      if (identical(event_times_i, event_times_j)) {  # Same process (diagonal)
        ...
        kernel_prod_mat_remove_diag <- kernel_prod_mat - diag(kernel_prod_mat)
