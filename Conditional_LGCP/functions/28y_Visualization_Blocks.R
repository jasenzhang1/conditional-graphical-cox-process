...
result_ROC_prep <- function(step_12, kept_names, kept_time_idx){
  ...
  for (i in seq_along(step_12)) {
    for (j in seq_along(step_12[[i]])) {
      ...
      plot_title <- paste('y=', y_names[i], ', ', x_names[j])
      
      if(is.data.frame(roc_df)){
...
result_44_prep <- function(step_3, step_4, p, ground_truth, X_truth, eigen_setting){
  ...
  if(eigen_setting %in% c('only_joint', 'trig_simple')){
    step_3_names <- rep(step_3_names, each = 1)
  } else if(eigen_setting %in% c('trig_and_joint')){
    step_3_names <- rep(step_3_names, each = 3) # for each eig1 eig2 eig3
  } else{
    stop('Invalid eigen_setting')
  }
